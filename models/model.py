...
class UMT(nn.Module):
    ...
    def forward(self, data, mode):
        ...
        if self.video_enc is not None:
            ...
            d_emb = r_emb = v_emb = self.video_enc(data['video'])
        else:
            v_emb = data['video']

        if self.audio_enc is not None:
            d_emb = r_emb = a_emb = self.audio_enc(data['audio'])
            # d_emb = r_emb = a_emb = self.audio_enc(data['audio'], mask=mask)
        else:
            a_emb = data['audio']

        if self.cross_enc is not None:
            d_emb = r_emb = self.cross_enc(v_emb, a_emb)
        ...
        output = dict(
            _avg_factor=1, _out=dict(meta=data.get('meta')))
        # output = dict(
